...
describe('adminAuth', () => {
  ...
  describe('when ADMIN_PASSWORD is unset', () => {
    ...
    it('rejects any password and any token', () => {
      expect(verifyPassword('anything')).toBe(false);
      expect(verifyToken('whatever.sig')).toBe(false);
    });
    it('throws when asked to issue a token', () => {
  ...
  describe('when ADMIN_PASSWORD is set', () => {
    ...
    it('invalidates tokens when the password changes (logout-everywhere)', () => {
      const token = issueToken();
